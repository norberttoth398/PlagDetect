...
def measure_bbox(img, sqrt_area = True):
    ...
    Args:
        ...
        sqrt_area (bool, optional): _description_. Defaults to True.

    Returns:
        _type_: _description_
    """
    #calculate contours
    cnt, _ = cv2.findContours(label.astype(np.uint8),cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def measure_ellipse(img, sqrt_area=True):
    ...
    #calculate aspect ratio
    aspect_ratio = np.asarray([major_axis[i]/minor_axis[i] for i in range(len(minor_axis))])
